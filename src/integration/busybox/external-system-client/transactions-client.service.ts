  ...
  async debitTransaction(data: TransactionDto): Promise<TransactionResponse> {
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.post<TransactionResponse>(`${this.configService.get('BUSY_BOX_API_BASE_URL')}/transaction/debit`, data, config)
      );
  ...
  async creditTransaction(data: TransactionDto): Promise<TransactionResponse> {
    ...
    try {
      const response: AxiosResponse<TransactionResponse> = await firstValueFrom(
        this.httpService.post<TransactionResponse>(`${this.configService.get('BUSY_BOX_API_BASE_URL')}/transaction/credit`, data, config)
      );
      return response.data;
    } catch (error) {
      throw error;

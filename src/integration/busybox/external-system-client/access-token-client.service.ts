...
import { BusyBoxAccessTokenCacheKey, BusyBoxAccessTokenTTL } from '../external/constants/access-token-cache-timeout.constant';
...
export class AccessTokenClientService {
  ...
  async getToken(): Promise<string> {
    const token = await this.cacheManager.get(BusyBoxAccessTokenCacheKey) as string;
    if (token) {
        return token;
    }
    ...
    try {
      ...
      await this.cacheManager.set('', response.data.token, BusyBoxAccessTokenTTL);
